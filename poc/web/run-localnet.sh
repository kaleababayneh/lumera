...
"$LD" keys add val "${KR[@]}" --algo eth_secp256k1 >/dev/null 2>&1
"$LD" keys add pub "${KR[@]}" --algo eth_secp256k1 >/dev/null 2>&1
VAL=$("$LD" keys show val -a "${KR[@]}"); PUB=$("$LD" keys show pub -a "${KR[@]}")
...
d['app_state']['incentives']['metric_snapshots']=[{
...
# Fund the publisher so it can escrow a tool bond.
